...
public class BailServiceImpl implements BailService {
  ...
  public List<OperationDto> closeBail(Long id) {
    ...
    Long lagence = 0L;
    if (id != null) {
      BailLocation newBailLocation = bailLocationRepository
        .findById(id)
        .orElse(null);
      if (newBailLocation == null) throw new EntityNotFoundException(
        "BailLocation from id not found",
        ErrorCodes.BAILLOCATION_NOT_FOUND
      );
      // Mise a jour de la table Operation
      Bienimmobilier bienLiberer = bienImmobilierService.findBienByBailEnCours(
        id
      );
      if (bienLiberer != null) {
      ...
      YearMonth periodOfCloture = YearMonth.of(
        dateCloture.getYear(),
        dateCloture.getMonth()
      );
      LocalDate initial = LocalDate.of(
        periodOfCloture.getYear(),
        periodOfCloture.getMonth(),
        1
      );
      System.out.println("La date de debut initial est: " + initial);
      LocalDate dateClotureEffectif = initial.withDayOfMonth(1);
      System.out.println(
        "La date de debut dateClotureEffectif est: " + dateClotureEffectif
      );
      // Determinons la liste des appels loyers dont la date de debut est superieur a
      // la dateClotureEffecture
      List<AppelLoyer> listeAppelLoyerAyantDateDebutSupDateCloture = appelLoyerRepository
        .findAll()
        .stream()
        .filter(appelLoyersFactureDto ->
          Objects.equals(
            appelLoyersFactureDto.getBailLocationAppelLoyer().getId(),
            newBailLocation.getId()
          )
        )
        .filter(appelLoyersFactureDto ->
        ...
        .filter(appelLoyersFactureDto ->
          appelLoyersFactureDto.getSoldeAppelLoyer() == montantBail
        )
  ...
  public int nombreBauxActifs(Long idAgence) {
    return (int) bailLocationRepository
      .findAll()
      .stream()
    ...
    Bienimmobilier bien = bienImmobilierRepository
      ...
      .orElseThrow(() ->
        new InvalidEntityException(
          "Aucun Bail has been found with Code " + id,
  ...
  public List<OperationDto> findAllByIdLocataire(Long id) {
    ...
    Utilisateur locataire = utilisateurRepository
      ...
      .orElseThrow(() ->
        new InvalidEntityException(
          "Aucun utilisateur has been found with Code " + id,
  ...
  public boolean deleteOperationById(Long id) {
    ...
    List<EncaissementPrincipalDTO> encaissement = encaissementRepository
      .findAll()
      .stream()
      .filter(operation ->
        operation
    ...
    if (!encaissement.isEmpty()) {
      throw new EntityNotFoundException(
        "L = " + id + " " + "Il existe des encaissement our ce bail",
        ErrorCodes.APPARTEMENT_NOT_FOUND
      );
    }
    Operation operation = bailLocationRepository
      .findById(id)
      .orElseThrow(() ->
        new EntityNotFoundException(
          ...
          id +
          " " +
          "Il existe des encaissement our ce bail",
          ErrorCodes.APPARTEMENT_NOT_FOUND
        )
      );
    Bienimmobilier bienAModifier = bienImmobilierRepository
      .findById(operation.getBienImmobilierOperation().getId())
      .get();
  ...
  public OperationDto modifierUnBail(BailModifDto dto) {
    BailLocation operation = bailLocationRepository
      .findById(dto.getIdBail())
      .orElseThrow(() ->
        new EntityNotFoundException(
          "Aucune Operation avec l'ID = " + dto.getIdBail(),
          ErrorCodes.APPARTEMENT_NOT_FOUND
        )
      );
    // MODIFIER LE BAIL
    ...
    boolean modifMontantLoyerbail = montantLoyerBailService.saveNewMontantLoyerBail(
      0L,
      dto.getNouveauMontantLoyer(),
      dto.getAncienMontantLoyer(),
      dto.getIdBail(),
      bailSave.getIdAgence(),
    ...
    if (modifMontantLoyerbail) {
      ...
      if (!loyers.isEmpty()) {
        for (int index = 0; index < loyers.size(); index++) {
          AppelLoyer lappelTrouver = appelLoyerRepository
            .findById(loyers.get(index).getId())
            .orElse(null);
          // lappelTrouver.setSoldeAppelLoyer(dto.getNouveauMontantLoyer());
          // if (lappelTrouver.getSoldeAppelLoyer() > 0) {
          lappelTrouver.setSoldeAppelLoyer(
            dto.getNouveauMontantLoyer() -
            (
              lappelTrouver.getMontantLoyerBailLPeriode() -
              lappelTrouver.getSoldeAppelLoyer()
            )
          );
          // }
          lappelTrouver.setMontantLoyerBailLPeriode(
            dto.getNouveauMontantLoyer()
          );
  ...
  public OperationDto findOperationById(Long id) {
    BailLocation findBailLocation = bailLocationRepository
      .findById(id)
      .orElseThrow(() ->
        new EntityNotFoundException(
          "Aucune Operation avec l'ID = " + id,
          ErrorCodes.BAILLOCATION_NOT_FOUND
        )
      );
  ...
  @Override
  public LocataireEncaisDTO bailBayLocataireEtBien(
    Long locataire,
    Long bienImmobilier
  ) {
    List<LocataireEncaisDTO> lesbeaux = bailLocationRepository
      .findAll()
      .stream()
      .filter(bien ->
        bien.getBienImmobilierOperation().getId() == bienImmobilier &&
        bien.getUtilisateurOperation().getId() == locataire
      )
  ...
  @Override
  public OperationDto prolongerUnBail(BailProlongeDto dto) {
    BailLocation operation = bailLocationRepository
      .findById(dto.getId())
      .orElseThrow(() ->
        new EntityNotFoundException(
          "Aucune Operation avec l'ID = " + dto.getId(),
          ErrorCodes.APPARTEMENT_NOT_FOUND
        )
      );
    // MODIFIER LE BAIL
    operation.setDateFin(dto.getDateFin());
    operation.setDateDebutProlong(dto.getDateDebut());
    operation.setDateFinProlong(dto.getDateFin());
    BailLocation bailSave = bailLocationRepository.save(operation);
                AppelLoyerRequestDto appelLoyerRequestDto = new AppelLoyerRequestDto();
            appelLoyerRequestDto.setIdBailLocation(bailSave.getId());
            appelLoyerRequestDto.setMontantLoyerEnCours(bailSave.getMontantLoyerBail().get(0).getNouveauMontantLoyer());
            appelLoyerRequestDto.setIdAgence(bailSave.getIdAgence());

            appelLoyerService.saveProlongAppel(appelLoyerRequestDto);
    return bailMapperImpl.fromOperation(bailSave);
  }
}
